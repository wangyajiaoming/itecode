...
void SceneInit(void)
{
    ...
    #ifdef CFG_ENABLE_ROTATE	//屏触控旋转角度设置
...
extern void ExternaAirDisplayInit(void);		//函数再温度探测定义
...
static void LoadScene(void)
{
    ...
    //ituSceneSetRotation(&theScene, ITU_ROT_90, ithLcdGetWidth(), ithLcdGetHeight());//屏幕触控旋转角度
	...
	ExternalUart1ProcessInit();

	ExternaAirDisplayInit();
#endif
}
...
static void CheckExternal(void)
{
   ...
   // int             ret = ExternalReceive(&ev);
	 int             ret = ExternalUart1Receive(&ev);
...
int SceneRun(void)
{
    ...
    for (;;)
    {
		...
		if(send_tempOrdehu_flag == true)		//在这做温度和湿度显示调用
		{
			send_tempOrdehu_flag = false;
//			Display_Temp_Dehu("airFastTmpText1",1);
//			Display_Temp_Dehu("airFastTmpText9",1);
			//Display_Temp_Dehu("airDehuText",0);
		}
        ...
        while (SDL_PollEvent(&ev))
        {
            ...
            case SDL_FINGERDOWN:
                ...
                printf("touch: down %d, %d\n", ev.tfinger.x, ev.tfinger.y);
                {
					...
					s_InductionTCB.vCharLed_Status=true;
					//===========================================================================	
                    mouseDownTick = SDL_GetTicks();
    #ifdef DOUBLE_KEY_ENABLE
